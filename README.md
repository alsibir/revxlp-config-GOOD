...
## 10 Лицензирование и ответственность

Лицензия и условия использования определяются файлами репозитория (например, `LICENSE`, если добавлен). Конфигурация предназначена для использования в рамках проекта `revxlp` и требует верификации на целевом оборудовании перед эксплуатацией.

## 11 Контакты и сопровождение

Сопровождение конфигурации выполняется через issues и pull requests GitHub репозитория. Для обращений рекомендуется прикладывать:

* лог сборки (CI или локальный);
* используемые версии ZMK/Zephyr;
